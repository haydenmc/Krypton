...
function clientPowerCheck(%client) {
	buildPowerList();
	%group = nameToID("MissionCleanup/Deployables");
	%count = %group.getCount();
	for(%i=0;%i<%count;%i++) {
...
function genLinkedObj(%powerObj,%obj) {
	if (%obj.powerFreq == %powerObj.powerFreq) {
		if (vectorDist(%obj.getPosition(),%powerObj.getPosition()) < %powerObj.powerRadius  //Changed from %powerObj.getDataBlock().powerRadius
...
function toggleGenerator(%obj,%state) {
	...
	if (%obj.isSwitchedOff && (%state != false || %state $= "")) {
		...
		setTargetName(%obj.target,addTaggedString("\c8"@%obj.label@"\c6"));
		...
		setTargetName(%obj.target,addTaggedString("Disabled \c8"@%obj.label@"\c6"));
		%obj.switchTime = getSimTime();
