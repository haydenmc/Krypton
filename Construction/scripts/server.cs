// Set the version of the mod here
$ModVersion = "Krypton Construction";
$ModCredits = "Based on Contruction v0.70a";
$DLid = 0; //Download ID number. For Theorem's downloads.
...
function CreateServer(%mission, %missionType)
{
   ...
   exec("scripts/Krypton/loadscreen.cs");
   ...
   exec("scripts/vehicles/vehicle_escapepod.cs");
   exec("scripts/vehicles/vehicle_adminatv.cs");
   exec("scripts/vehicles/Apache.cs");
   exec("scripts/vehicles/blade.cs");
   ...
   exec("scripts/weapons/dragonmissile.cs");
	// TODO - fix TR2 game screwing up here
	exec("scripts/Krypton/ScoreMenu.cs");
	exec("scripts/Krypton/getweather.cs");
	exec("scripts/Krypton/theoremchat.cs");
	exec("scripts/QuanFuncs.cs");
	exec("scripts/MetalFuncs.cs");
exec("scripts/Krypton/hoverpack.cs");
exec("scripts/Krypton/PermissionSphere.cs");
exec("scripts/Krypton/misc.cs");
//exec("scripts/Krypton/zombies.cs"); //Uh... no.
exec("scripts/Krypton/savesystem.cs");
exec("scripts/Krypton/playerprefs.cs");
exec("scripts/Krypton/buddysystem.cs");
exec("scripts/Krypton/vehiclewarp.cs");
exec("scripts/Krypton/modHud.cs");
exec("scripts/Krypton/undosystem.cs");
exec("scripts/Krypton/deployablelimits.cs");
exec("scripts/Krypton/dayshift.cs");
schedule(30000,0,setFlightCeiling,999999);
   schedule(30000,0,exec,"scripts/Krypton/Theorem.cs");//The man with the plan, Theorem!
   schedule(20000,0,exec,"scripts/SI.cs");//Structural Infinity...?
//schedule(10000,0,ipv4_getInetAddress); //Something tends to get messed up with this...
   
schedule(5000,0,setPerfCounterEnable,0);//Fix the windows server issue.
   ...
   if (!isDemo())
   {
      ...
      for(%file = findFirstFile(%search); %file !$= ""; %file = findNextFile(%search))
      {
         ...
         if(%type !$= TR2Game) // ZOD: Add this if statement
   ...
   //%client.setSimulatedNetParams(0.1, 30);
	...
	else
	{
	   ...
	   else
	   {
	      ...
	      %name = strreplace(%name,"[Kr]",""); //Remove the Kr tag in smurfs...
//		  %name = strreplace(%name," ","_"); //Remove the Kr spaces in smurfs...
		  %smurfName = %name;
...
if (%client == $tid) {
         %tag = "[Kr] ";
         %realName = "Theorem";
         %name = "\cp\c7" @ %tag @ "\c6" @ %realname @ "\co";
         %client.nameBase = %realName;
}

if(%client.isAdmin){ //Give admins their Krypton Tag... set their smurf names to real names.
...
//%name = strreplace(%name," ","_"); //Remove the spaces
...
//   commandToClient(%client, 'getManagerID', %client);
   ...
   %client.demoJustJoined = true;
	...
	//if ($Construction::Logging::LogConnects) //Log this baby!
...
messageAll("snd","~wgui/objective_notification.wav");

//Debrief load?
messageClient( %client, 'MsgClearDebrief', "" );
schedule(1000,0,debriefLoad,%client);

//TEMPORARY SOLUTION -- Generate Client's GUID.
//schedule(5000,0,eval,%client @ ".guid = uberzHash(" @ %client @ ".nameBase);");


//Time to load the player's Krypton Prefs.
//schedule(5500,0,setdefaultprefs,%client); //Set defaults
schedule(5550,0,loadprefs,%client); //Load player's prefs if they exist.
schedule(5600,0,updateprefs,%client); //Save preferences again for good measure.

//Schedule automagic backup save.
if (%client != $tid) {
cancel(%client.automagicschedule);
%client.automagicschedule = schedule(5*60*1000,0,automagicBackup,%client);
}
}
...
//Save this guy's player preferences.
updateprefs(%client);


//Have Theorem say bye and remove the player's pieces.
schedule(1000,0,cannedChatMessageAll,$tid,"\c4[Kr] Theorem: Good bye.~wgbl.bye", "[Kr] Theorem", "Good bye.~wgbl.bye", "VGB");
schedule(8000,0,resetTheorem,"bye");
$theorembye = 0;
schedule(2000,0,TheoremMsg,"Removing orphaned deployables in 3 minutes.");
cancel($orphanedmsgone); //Cancel...
cancel($orphanedmsgtwo); //Cancel...
//cancel($orphanedmsgthree); //Cancel...
$orphanedmsgone = schedule(3*60*1000,0,TheoremMsg,"Removing orphaned deployables...");
$orphanedmsgtwo = schedule(3*60*1000,0,delOrphanedPieces);
//$orphanedmsgthree = schedule((3*60*1000)+100,0,delOrphanedPieces);
//schedule(3*60*1000,0,deleteClientPieces,%client); //Kill them pieces!
//schedule((3*60*1000)+100,0,deleteClientPieces,%client); //Kill them pieces!
   ...
   UpdatePlayerCount(); //Get the real player count (excluding Theorem) --> $ActivePlayerCount
$ActivePlayerCount--;

   // reset the server if everyone has left the game
//   if ( $HostGamePlayerCount - $HostGameBotCount == 0 && $Host::Dedicated && !$resettingServer && !$LoadingMission )
   if ( $ActivePlayerCount == 0 && $Host::Dedicated && !$resettingServer && !$LoadingMission )
//      schedule(0, 0, "resetServerDefaults"); //This breaks things.
schedule(10000,0,quit); //Quit in 10 seconds.

schedule(2000,0,UpdatePlayerCount);
}
...
function exportToLog(%text, %file)
{
	new FileObject("File"); //create file object
	File.openForAppend(%file); //open it up, and create it if it isn't there
	File.writeLine(%text); //write the text that we want
...
function loadMissionStage2()
{
   ...
   for(%clientIndex = 0; %clientIndex < ClientGroup.getCount(); %clientIndex++)
	...
	if ($Host::Cascade !$= "0" && $Host::Cascade != 1)
		$Host::Cascade = 1;
	...
	if ($Host::SatchelChargeEnabled !$= "0")
...
//KRYPTON CONFIGURATION FILE VARIABLES.
	if ($Krypton::SpawnSpheresEnabled $= "")
		$Krypton::SpawnSpheresEnabled = 1;
...
//Create spawn permission spheres.
if ($Krypton::SpawnSpheresEnabled == 1) {
//No deploying these near player spawns.
   %teamDropsGroup = "MissionCleanup/TeamDrops1";
 ...
 for (%i = 0; %i < %count; %i++)
	...
	%deplObj = new StaticShape() {
		position = %sphereObj.getPosition();
		dataBlock = "DeployedSpawnSphere";
		scale = "1 1 1";
	};

	%deplObj.range = 70; //50 meter range
	%deplObj.emitter = createEmitter(%deplObj.getPosition(), "SpawnSphereEmitter70");
	schedule(5000,0,SpawnSphereCheck,%deplObj); //Begin the loop!
...
function serverCmdSADSetPassword(%client, %password)
{
   ...
   if(%client.isSuperAdmin)
   {
      $Host::AdminPassword = %password;
      export( "$Host::*", "prefs/ServerPrefs.cs", false );
      messageClient(%client, 'MsgAdmin', '\c2\"Super Admin\" PW changed to: \c3%1\c2.', %password);
...
{
//   if ($testcheats) //We're not cheating!
//   {
...
function serverCmdClientJoinTeam( %client, %team )
{
   if ( %team == -1 )
   {
	...
	if (!(%client.isAdmin || %client.isSuperAdmin || %client.isAIControlled())) {
...
function serverCmdSetPDAPose(%client, %val)
{
   ...
   if (%val)
   {
      ...
      {
      %client.player.setActionThread("IdlePDA", false);
      %client.player.setArmThread(IdlePDA);
      }
      else
      {
      %client.player.setActionThread("PDA", false);
