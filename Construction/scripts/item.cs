...
//Apex: This should not differentiate between different types of deployables -- everything should work in the same fashion.
function cyclePackSetting(%plyr,%val) {
	...
	else if (%plyr.hasPermSphere) {
		%plyr.packSet = %plyr.packSet + %val;
		if (%plyr.packSet > $packSettings["permsphere"])
			%plyr.packSet = 0;
		if (%plyr.packSet < 0)
			%plyr.packSet = $packSettings["permsphere"];
		%line = $packSetting["permsphere",%plyr.packSet];
		bottomPrint(%plyr.client,"Sphere Type: " @ %line ,2,1);
		%changed = true;
	}
	else if (%plyr.hasProxSwitch) {
		%plyr.packSet = %plyr.packSet + %val;
		if (%plyr.packSet > $packSettings["proxswitch"])
			%plyr.packSet = 0;
		if (%plyr.packSet < 0)
			%plyr.packSet = $packSettings["proxswitch"];
		%line = $packSetting["proxswitch",%plyr.packSet];
		bottomPrint(%plyr.client,"Proximity Switch Range: " @ %line ,2,1);
		%changed = true;
	}
	else if (%plyr.hasBaseTurret) {
		%plyr.packSet = %plyr.packSet + %val;
		if (%plyr.packSet > $packSettings["baseturret"])
			%plyr.packSet = 0;
		if (%plyr.packSet < 0)
			%plyr.packSet = $packSettings["baseturret"];
		%line = $packSetting["baseturret",%plyr.packSet];
		bottomPrint(%plyr.client,"Turret Mode set to: " @ %line ,2,1);
		%changed = true;
	}
