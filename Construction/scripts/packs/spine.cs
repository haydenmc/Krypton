...
function spineDeployableImage::setModHud(%data,%client) {
%heightlist = "";
%currentsetting = 0;
%totalsettings = $packSettings["spine"];
%loopnum = 0;
%maxreached = 0;
%currentsetting = 0;
...
%setting = $packSetting["spine",%currentsetting];
...
%thecommand = "commandToClient(" @ %client @ ", 'ModHudPopulate', " @ "\"Beam Length-" @ %loopnum @ "\", " @ %heightlist @ ");";
eval(%thecommand);
echo(%thecommand);
%loopnum++;
}
schedule(200,0,eval,%client @ "." @ "player" @ ".packSet = 0;");
}

function spineDeployableImage::getModHud(%data,%client,%option,%value) {

%packset = ((%option-1)*10)+%value-1;
%client.player.packSet = %packset;

%setting = $packSetting["spine",%packset];
%setting = getWords(%setting,3,getWordCount(%setting));

messageClient( %client, 'MsgModHud', "\c2LSB Length Set To: " @ %setting );
}