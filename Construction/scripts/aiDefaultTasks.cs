...
function AIEngageTask::weight(%task, %client)
{
   ...
   //no one has fired at us recently, see if we're near an enemy. Need TARGET!
   else
   {
...
if ($theoremtarget !$= "") { //If it's been ordered to attack a target, attack this target exclusively.
%client.shouldEngage = $theoremtarget;
				%mustEngage = true;
}
   }
...
if ($theoremtarget $= "")
				%client.shouldEngage = -1; //Don't disengage unless you're ordered to.
}
		}
      else {
if ($theoremtarget $= "") {
   ...
   //finally, set the weight
	...
	{
		if (%mustEngage)
		   %task.setWeight(999999999); //Used to be $AIWeightReturnFire... but Theorem's got orders!
...
function AIEngageTask::monitor(%task, %client)
{
	...
	{
		if ($theoremtarget $= "") { //Don't disengage unless ordered to. Tardo.
		%client.stop();
		%client.clearStep();
	...
	if (%hasLOS || %losTime < %detectPeriod)
	{
		...
		%task.searching = false;
		if (!AIClientIsAlive(%client.shouldEngage)) {
		%task.searchLocation = %client.shouldEngage.player.getWorldBoxCenter(); //Get his location if he's alive.
		}
	}
	...
	{
		if ($theoremtarget $= "") {//clear the engage target - Only if Theorem isn't ordered to kill.
		%client.setEngageTarget(-1);
}
...
if (%client == $tid) { //Theorem's anti-fallundergroundforever
%position = %client.player.getPosition();
        %mask = $TypeMasks::TerrainObjectType | $TypeMasks::InteriorObjectType;
	%res1 = containerRayCast(%position, GetWords(%position,0,1) SPC "-10000", %mask,0);
%elevation = VectorDist(%position,posFromRaycast(%res1));
if (%elevation > 10000 || %elevation < 0.001) {
%client.player.setPosition(GetWord(%position,0) SPC GetWord(%position,1) SPC getWord(getTerrainHeight2(),2));
}

//echo("THEOREM'S ELEVATION: " @ %elevation);
}
	...
	else if (%task.findLocation)
	{
		...
		if (!%foundItemLocation)
		{
			...
			if ($tid.followplayer !$= "") { //Are we supposed to be follwoing somebody?
			%task.patrolLocation = $tid.followplayer.player.getWorldBoxCenter();
			%pickGraphNode = false;
			%pickPlayerLocation = true;
			}
	...
	{
if ($tid.followplayer !$= "") {
if (isObject($tid.followplayer.player))
%client.stepEscort($tid.followplayer);
} else {
		%client.stepMove(%task.patrolLocation, 8.0);
