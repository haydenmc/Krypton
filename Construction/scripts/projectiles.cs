...
{
   if(isObject(%targetObject) && %targetObject.inpeacesphere $= "") // Console spam fix - ToS. z0ddm0d
   {
...
if (%targetObject.inpeacesphere !$= "") //No hurting people in peace spheres.
return;
...
//      %obj.setInvincible( false ); // fire your weapon and your invincibility goes away.
		...
		// Extra safety
		if (%obj.inpeacesphere !$= "") { //It's a peace sphere, and we're in its range.
//			jailPlayer(%obj.client,false,3*60); //Jail 'em for 6 minutes.
//			messageClient(%obj.client,'msgClient','\c2You will do 3 minutes in jail for firing in a peace zone.');
//			messageAllExcept(%obj.client,-1,'msgClient','\c2%1 will do 3 minutes in jail for firing in a peace zone.',%obj.client.nameBase);
%dataBlockName = %data.getName();
if (%dataBlockName !$= "ConstructionTargeter" && %dataBlockName !$= "ConstructionToolImage" && %dataBlockName !$= "ConstructionScopeImage" &&  %dataBlockName !$= "TargetingLaserImage") {
return;
}
			}
	}
...
//echo(%explosionSource.getDataBlock().getName());
   %numTargets = 0;
...
if (%explosionSource.getDataBlock().getName() $= "BasicConcGrenade" && %data.getClassName() $= "PlayerData") {
