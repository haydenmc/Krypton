...
datablock StationFXPersonalData( PersonalInvFX )
{
   delay = 0;
   fadeDelay = 0.5;
   lifetime = 2; //used to be 1.2
   height = 2.75; //used to be 2.5
   numArcSegments = 10.0;
   numDegrees = 180.0;
   trailFadeTime = 1; //used to be 0.5
   leftRadius = 1.85; //used to be 1.85
   rightRadius = 1.85; //used to be 1.85
   leftNodeName = "FX1";
   rightNodeName = "FX2";

   texture[0] = "skins/disc_muzzle";
};
...
function StationInventory::setPlayersPosition(%data, %obj, %trigger, %colObj)
{
   ...
   if((VectorLen(%vel) < 22) && (%obj.triggeredBy != %colObj))
   {
	...
	//Took all of the above out to have some freedom.
...
function StationVehiclePad::createStationVehicle(%data, %obj)
{
   ...
   if(%obj.ApacheFlyer !$= "Removed")
	%sv.vehicle[ApacheFlyer] = true;
   if(%obj.KryptonBlade !$= "Removed")
	%sv.vehicle[KryptonBlade] = true;
}
// End z0dd - ZOD - Founder
// -----------------------------------------------------------------------------
