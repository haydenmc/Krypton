...
echo("saving buildings for GUID " @ %cl.guid);
	if (%quiet == true) {
	...
	Building.writeLine("// Construction " @ $Version);
	Building.writeLine("");
	...
	while((%obj = containerSearchNext()) != 0) {
		%cmp = writeBuildingComponent(%obj,%cl.guid);
			echo("Writing componant2 for GUIDOBJ " @ %obj.ownerGUID);
		if (%cmp !$= "") {
			Building.writeLine(%cmp);
		}
	}
    if($Ele[0] !$= "")
	   Building.writeLine("ConnectEles();");
	...
	//Building.writeLine("" @ globalPowerCheck() @ "\";");
	Building.close();
	Building.delete();
	return %file;
}

function writeBuildingComponent(%obj,%theguid) {
	%dataBlockName = %obj.getDatablock().getName();
	%save = false;
	%save = saveBuildingCheck(%obj);
	if (%save == true && (%theguid $= %obj.ownerGUID)) {
		...
		if (%obj.powerRadius !$= "") %buildingPiece = %buildingPiece @ "powerRadius = \"" @ %obj.powerRadius @ "\";";
		if (%obj.isSwitchedOff !$= "") %buildingPiece = %buildingPiece @ "isSwitchedOff = \"" @ %obj.isSwitchedOff @ "\";";
		if (%obj.switchTimedDelay !$= "") %buildingPiece = %buildingPiece @ "switchTimedDelay = \"" @ %obj.switchTimedDelay @ "\";";
...
//Some pack dispenser stuff...
if (%obj.set1 !$= "") %buildingPiece = %buildingPiece @ "set1 = \"" @ %obj.set1 @ "\";";
...
if (%obj.packBlock !$= "") %buildingPiece = %buildingPiece @ "packblock = \"" @ %obj.packblock @ "\";";
		...
		//Door stuff... Not added already for some reason
        ...
        if (%obj.canlock !$= "") %buildingPiece = %buildingPiece @ "canlock = \"" @ %obj.canlock @ "\";";
	...
	if (%obj.isDoor !$= "") %buildingPiece = %buildingPiece @ "isDoor = \"" @ %obj.isDoor @ "\";";
		...
		//Decoy pack stuff
		if (%obj.decoy !$= "") %buildingPiece = %buildingPiece @ "decoy = \"" @ %obj.decoy @ "\";";
		if (%obj.decoyType !$= "") %buildingPiece = %buildingPiece @ "decoyType = \"" @ %obj.decoyType @ "\";";
		if (%obj.decoySkin !$= "") %buildingPiece = %buildingPiece @ "decoySkin = \"" @ %obj.decoySkin @ "\";";
		if (%obj.decoyPack !$= "") %buildingPiece = %buildingPiece @ "decoyPack = \"" @ %obj.decoyPack @ "\";";
		if (%obj.decoyWeapon !$= "") %buildingPiece = %buildingPiece @ "decoyWeapon = \"" @ %obj.decoyWeapon @ "\";";
		if (%obj.decoyEmote !$= "") %buildingPiece = %buildingPiece @ "decoyEmote = \"" @ %obj.decoyEmote @ "\";";
		if (%obj.decoyName !$= "") %buildingPiece = %buildingPiece @ "decoyName = \"" @ %obj.decoyName @ "\";";	
		if (%obj.mode !$= "") %buildingPiece = %buildingPiece @ "mode = \"" @ %obj.mode @ "\";";	
...
// Ugly, but it works. May be cleaned up later.
		...
		else if (%dataBlockName $= "DeployedEditorPack") { //Writes out long-ass lines...
		for (%i=0;%i<%obj.slotcount;%i++)
		{
		if (!IsObject(%obj.slot[%i]))
		break;
		%buildingpiece = %buildingpiece @ "%edit = new (staticshape) () {";
		%buildingpiece = %buildingpiece @ "datablock = "@%obj.slot[%i].getdatablock().getname()@";";
		%buildingpiece = %buildingpiece @ "position = \""@%obj.slot[%i].getposition()@"\";";
		%buildingpiece = %buildingpiece @ "rotation = \""@%obj.slot[%i].getrotation()@"\";";
		%buildingpiece = %buildingpiece @ "needsfit = \""@%obj.slot[%i].needsfit@"\";";
		%buildingpiece = %buildingpiece @ "grounded = \""@%obj.slot[%i].grounded@"\";";
		%buildingpiece = %buildingpiece @ "deployed = \""@%obj.slot[%i].deployed@"\";";
		%buildingpiece = %buildingpiece @ "istscale = \""@%obj.slot[%i].istscale@"\";";
		%buildingpiece = %buildingpiece @ "isescaled = false;";
		%buildingpiece = %buildingpiece @ "oldscale = \""@%obj.slot[%i].getscale()@"\";";
		%buildingpiece = %buildingpiece @ "escale = \""@%obj.slot[%i].escale@"\";";
		%buildingpiece = %buildingpiece @ "istfade = \""@%obj.slot[%i].istfade@"\";";
		%buildingpiece = %buildingpiece @ "isefaded = false;";
		%buildingpiece = %buildingpiece @ "isthide = \""@%obj.slot[%i].isthide@"\";";
		%buildingpiece = %buildingpiece @ "isehidden = false;";
		%buildingpiece = %buildingpiece @ "istcloak = \""@%obj.slot[%i].istcloak@"\";";
		%buildingpiece = %buildingpiece @ "isecloaked = false;";
		%buildingpiece = %buildingpiece @ "istname = \""@%obj.slot[%i].istname@"\";";
		%buildingpiece = %buildingpiece @ "isenamed = false;";
		%buildingpiece = %buildingpiece @ "oldname = \""@%obj.slot[%i].nametag@"\";";
		%buildingpiece = %buildingpiece @ "ename = \""@%obj.slot[%i].ename@"\";";
		%buildingpiece = %buildingpiece @ "escale = \""@%obj.slot[%i].escale@"\";";
		%buildingpiece = %buildingpiece @ "powerfreq = \""@%obj.slot[%i].powerfreq@"\";";
		%buildingpiece = %buildingpiece @ "editor = %building;";
		%buildingpiece = %buildingpiece @ "slot = \""@%obj.slot[%i].slot@"\";";
		%buildingpiece = %buildingpiece @ "target = \""@%obj.slot[%i].gettarget()@"\";};";
		%buildingpiece = %buildingpiece @ "setTargetSensorGroup(%edit.getTarget(),%building.owner.team);";
		%buildingpiece = %buildingpiece @ "addtodeploygroup(%edit);";
		%buildingpiece = %buildingpiece @ "%building.slot["@%i@"] = %edit;";
		}
		%buildingPiece = %buildingPiece @ "%building.slotcount = "@%obj.slotcount@";";
      		if (!$SaveBuilding::Quiet)
		echo("Saving: " @ %obj @ " Name: " @ %datablockname );
       }
	}
...
function writeBuildingComponentCentered(%obj) {
	...
	if (%save == true) {
		...
		if (%obj.deployed !$= "") %buildingPiece = %buildingPiece @ "deployed = \"" @ %obj.deployed @ "\";";
		if (%obj.impulse !$= "") %buildingPiece = %buildingPiece @ "impulse = \"" @ %obj.impulse @ "\";";
		if (%obj.proj !$= "") %buildingPiece = %buildingPiece @ "proj = \"" @ %obj.proj @ "\";";
		if (%obj.projtype !$= "") %buildingPiece = %buildingPiece @ "projtype = \"" @ %obj.projtype @ "\";";
		if (%obj.velocityMod !$= "") %buildingPiece = %buildingPiece @ "velocityMod = \"" @ %obj.velocityMod @ "\";";
		if (%obj.toggletype !$= "") %buildingPiece = %buildingPiece @ "toggletype = \"" @ %obj.toggletype @ "\";";
		if (%obj.powercontrol !$= "") %buildingPiece = %buildingPiece @ "powercontrol = \"" @ %obj.powercontrol @ "\";";
		...
		if (%obj.tripMode !$= "") %buildingPiece = %buildingPiece @ "tripMode = \"" @ %obj.tripMode @ "\";";
		if (%obj.fieldMode !$= "") %buildingPiece = %buildingPiece @ "fieldMode = \"" @ %obj.fieldMode @ "\";";
		if (%obj.repeatTime !$= "") %buildingPiece = %buildingPiece @ "repeatTime = \"" @ %obj.repeatTime @ "\";";
		if (%obj.rounds !$= "") %buildingPiece = %buildingPiece @ "rounds = \"" @ %obj.rounds @ "\";";
		if (%obj.waypoint !$= "") %buildingPiece = %buildingPiece @ "waypoint = \"" @ %obj.waypoint @ "\";";
		if (%obj.password !$= "") %buildingPiece = %buildingPiece @ "password = \"" @ %obj.password @ "\";";
     	if (%obj.repeatTime !$= "") %buildingPiece = %buildingPiece @ "repeatTime = \"" @ %obj.repeatTime @ "\";";
  	    if (%obj.time !$= "") %buildingPiece = %buildingPiece @ "time = \"" @ %obj.time @ "\";";
		if (%obj.isNamed !$= "") %buildingPiece = %buildingPiece @ "isNamed = \"" @ %obj.isNamed @ "\";";
		if (%obj.namedName !$= "") %buildingPiece = %buildingPiece @ "namedName = \"" @ %obj.namedName @ "\";";
		%buildingPiece = %buildingPiece @ "};";
		$SaveBuilding::Saved++;
	}
	else {
...
function loadBuilding(%file) {
	...
	exec($SaveBuilding::SaveFolder @ %file);
 	compile($SaveBuilding::SaveFolder2 @ %file); // Just in case it bombs out
	exec($SaveBuilding::SaveFolder2 @ %file);
}
...
function saveBuildingTimer(%timeInSeconds,%globalEcho,%file,%useMultipleFiles,%isScheduled,%threadCount) {
	...
	if ($SaveBuilding::TimerEnabled == true && $SaveBuilding::TimerCount == %threadCount && isObject("MissionCleanup/Deployables")) {
		...
		if ($SaveBuilding::TimerGlobalChatEcho)
			cannedChatMessageAll($tid,"\c4{s}Theorem: Buildings saved. Next save in " @ %timeToNextSave, "{s}Theorem", "\c4{s}Theorem: Buildings saved. Next save in " @ %timeToNextSave, "BOT");
...
function saveBuildings(%name,%bool,%player,%rad) {
//saveBuilding("","",%a,0);
if(%bool $= "")
   %bool = 0;
%cl = PlayerNameTOObject(%player);
if(%cl !$= 0) {
   %rad = %radius;
}
else
...
%name2 = "Buildings/" @ %name @ "";

if(%bool)
saveBuildingCentered(%cl,%radius,%name2,0,0);
else
saveBuilding(%cl,%radius,%name,0,0);
}

function saveClientBuildings(%player,%name) {
//saveBuilding("","",%a,0);
   %bool = 0;
%cl = %player;
   %radius = "";
...
%name2 = "" @ %player.guid @ "/" @ %name @ ".cs" @ "";
echo(%name2);
saveBuilding(%cl,%radius,%name2,0,0);
}

function fo(%obj) { // fo = find owner
%guid = %obj.ownerGUID;
%count = ClientGroup.getCount();
for(%i=0;%i<%count;%i++) {
   if(ClientGroup.getObject(%i).guid $= %guid) {
       %obj.setOwner(ClientGroup.getObject(%i));
       %obj.owner = ClientGroup.getObject(%i);
       %obj.ownerGUID = ClientGroup.getObject(%i).guid;
   }
}
}
