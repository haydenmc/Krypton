//Version 2 CRAP
//Version2 physics model

$v2GravityForce = 			"-23.00";
$v2MassModifier = 			"1.00";

$v2JetForceLMH = 				"32.00 31.00 29.00";		//add to negative gravity

//$v2HorizSpeedLMH = 			"117.0 98.50 78.00";		//horiz speed barrier (hard)
//$v2HorizSpeedLMH = 			"98.50 92.50 78.00";		//horiz speed barrier (hard)
$v2HorizSpeedLMH = 			"88.50 82.50 68.00";		//horiz speed barrier (hard)
$v2HorizResistSpeedLMH = 	"62.00 58.00 43.00";		//horiz speed barrier (soft)
$v2HorizResistLMH = 			"0.400 0.400 0.400";		//horiz drag when exceeding soft barrier

$v2UpSpeedLMH = 				"117.0 98.50 78.00";		//vert speed barrier (hard)
$v2UpResistSpeedLMH = 		"30.00 30.00 25.00";		//vert speed barrier (soft)
$v2UpResistLMH	= 				"00.27 00.27 00.27";		//vert drag when exceeding soft barrier		//v2h 0.3->0.27

$v2MaxFwdJetSpeedLMH	=		"48.00 45.00 23.00";		//horiz speed from jet thrust
...
// Jet data
//----------------------------------------------------------------------------
datablock ParticleData(AdminArmorJetParticle)
{
   dragCoefficient      = 0.5;  //Was 0.0, original game shiped as 1.5
   gravityCoefficient   = 0;
   inheritedVelFactor   = 0.2;
   constantAcceleration = 0.0;
   lifetimeMS           = 150;  //Was a watered down 100, Original game was 150
   lifetimeVarianceMS   = 100;
   textureName          = "special/tracer00";
   colors[0]     = "0.95 0.85 0.0 0.6";
   colors[1]     = "1.0 0.0 0.0 0";
   sizes[0]      = 0.60;
   sizes[1]      = 0.05;
};

datablock ParticleEmitterData(AdminArmorJetEmitter)
{
   ejectionPeriodMS = 4;
   periodVarianceMS = 0;
   ejectionVelocity = 2;
   ...
   overrideAdvances = false;
   orientParticles  = true;
   particles = "AdminArmorJetParticle";
};
...
datablock PlayerData(LightMaleHumanArmor) : LightPlayerDamageProfile
{
   ...
   mass = 90 * $v2MassModifier;
   drag = 0.03;		//v2g=0.04;//0.275;		//v2 was 0.275, then .001 couldnt see a change
   maxdrag = 0.04;	//v2g=0.05;//0.33;	//v2 was 0.4
   density = 9;		//V2 TEST
   maxDamage = 0.66;
   maxEnergy =  60;
   repairRate = 0.0033;
   ...
   rechargeRate = 0.256;
   jetForce = 					getword($v2JetForceLMH, 0) * 90;				//v2 26.21 * 90;
   underwaterJetForce = 	getword($v2JetForceLMH, 0) * 90 * 0.8;// * 2;
   underwaterVertJetFactor = 1;//1.5;
   //jetEnergyDrain =  0.81;		//v2 0.796;		//v2 mod was .8
   jetEnergyDrain =  0.0;		//v2 0.796;		//v2 mod was .8
   underwaterJetEnergyDrain = 0.81;//0.5;
   minJetEnergy = 1;
   maxJetHorizontalPercentage = 1.1;		//v2 was .8

   runForce = 55.20 * 90;
   runEnergyDrain = 0;
   minRunEnergy = 0;
   maxForwardSpeed = 	15;		//v2 was 15
   maxBackwardSpeed = 	13;		//v2 was 13
   maxSideSpeed = 		13;		//v2 was 13

   maxUnderwaterForwardSpeed = 	12.65;		//v2 was 11.0
   ...
   horizMaxSpeed = 			getword($v2HorizSpeedLMH, 0);
   horizResistSpeed = 	getword($v2HorizResistSpeedLMH, 0);		//50;
   horizResistFactor = 	getword($v2HorizResistLMH, 0);		//v2 0.13;		//v2mod was .35, then 0.13
   maxJetForwardSpeed = getword($v2MaxFwdJetSpeedLMH, 0);	//v2 mod was 30, then 40

   upMaxSpeed = 				getword($v2UpSpeedLMH, 0);		//v2 80;
   upResistSpeed = 			getword($v2UpResistSpeedLMH, 0);
   upResistFactor = 		getword($v2UpResistLMH, 0);	//0.02;		//v2 20;		//v2 mod was .33, then .10

   minImpactSpeed = 60;		//G=45;
   speedDamageScale = 0.003;//v2 4	
   ...
   pickupRadius = 3;	//v2 was 0.75, then 1.2, then 2
   ...
   dustEmitter = LiftoffDustEmitter;		//v2
   ...
   groundImpactMinSpeed    = 20;		//v2g=10.0;
...
//ADMIN ARMOR -------------------------------------------------
datablock PlayerData(AdminMaleHumanArmor) : HeavyPlayerDamageProfile
{
   emap = true;
   ...
   hudImageNameFriendly[0] = "gui/hud_armbaricon";
   hudImageNameEnemy[0] = "gui/hud_armbaricon";
   hudRenderModulated[0] = true;
   ...
   maxEnergy =  999;
   repairRate = 0.1;
   energyPerDamagePoint = 0.0; // shield energy required to block one point of damage

   rechargeRate = 0.99;
   jetForce = 28.47 * 180;
   underwaterJetForce = 24.47 * 180 * 1.5;
   underwaterVertJetFactor = 1.5;
   jetEnergyDrain =  0.0;
   ...
   horizMaxSpeed = 68;
   horizResistSpeed = 33;
   horizResistFactor = 0.05;
   maxJetForwardSpeed = 60;
   ...
   minImpactSpeed = 45;
   speedDamageScale = 0.000;

   jetSound = ArmorJetSound;
   wetJetSound = ArmorJetSound;
   jetEmitter = AdminArmorJetEmitter;
   ...
   maxMines = 0;              // Max number of different mines the player can have
	...
	max[SniperRifle]		= 1;
	max[GrenadeLauncher]		= 1;
	max[GrenadeLauncherAmmo]	= 999;
	max[Mortar]			= 1;
	max[MortarAmmo]			= 999;
	max[MissileLauncher]		= 1;
	max[MissileLauncherAmmo]	= 999;
	max[Chaingun]			= 1;
	max[ChaingunAmmo]		= 999;
	max[RepairGun]			= 1;
	max[CloakingPack]		= 1;
	...
	max[TurretOutdoorDeployable]	= 1;
	max[TurretIndoorDeployable]	= 1;
	max[FlashGrenade]		= 999;
	max[ConcussionGrenade]		= 999;
	max[FlareGrenade]		= 5;
	max[TargetingLaser]		= 1;
	max[ELFGun]			= 0;
	max[ShockLance]			= 1;
...
//	max[DecoyDeployable] = 1;
max[repairTurretDeployable]	= 1;
max[Nuke] = 1;
max[PermSphereDeployable] = 1;
max[EditorPackDeployable] = 1;
max[waypointDeployable] = 1;
max[Shotgun] = 1;
max[ShotgunAmmo] = 999;
max[ConcGrenadeLauncher] = 1;
max[ConcGrenadeLauncherAmmo] = 999;
max[MiniAA] = 1;
max[SmokeGrenadeLauncher] = 1;
max[SmokeGrenadeLauncherAmmo] = 999;
max[Assault] = 1;
max[AssaultAmmo] = 999;
max[Buster] = 1;
max[BusterAmmo] = 999;
max[ProximitySwitch]     = 1;
max[MultiTool]     = 1;
max[Photon] = 1;
max[PhotonAmmo] = 999;
max[MpmFuelPack]		= 1;
...
max[ShotgunAmmo] = 30;
max[ConcGrenadeLauncher] = 1;
max[ConcGrenadeLauncherAmmo] = 15;
max[MiniAA] = 1;
max[SmokeGrenadeLauncher] = 1;
max[SmokeGrenadeLauncherAmmo] = 3;
max[Assault] = 1;
max[AssaultAmmo] = 600;
max[Buster] = 1;
max[BusterAmmo] = 20;
...
datablock PlayerData(VIPMaleHumanArmor) : PureMaleHumanArmor
{
   hudImageNameFriendly[0] = "gui/hud_plasma";
   hudImageNameEnemy[0] = "gui/hud_plasma";
...
datablock PlayerData(VIPFemaleHumanArmor) : VIPMaleHumanArmor
{
   shapeFile = "light_female.dts";
   waterBreathSound = WaterBreathFemaleSound;
   jetEffect =  HumanMediumArmorJetEffect;
...
datablock PlayerData(VIPMaleBiodermArmor) : VIPMaleHumanArmor
{
   shapeFile = "bioderm_light.dts";
   jetEmitter = BiodermArmorJetEmitter;
   jetEffect =  BiodermArmorJetEffect;
...
function Armor::onDisabled(%this,%obj,%state)
{
   ...
   if(%obj.getdatablock().getname() $= "ZombieArmor" || %obj.getdatablock().getname() $= "FZombieArmor" || %obj.getdatablock().getname() $= "LordZombieArmor" || %obj.getdatablock().getname() $= "DemonZombieArmor" || %obj.getdatablock().getname() $= "RapierZombieArmor" || %obj.getdatablock().getname() $= "DemonMotherZombieArmor"){
   $ZombieNum--;
	%dodeath = (getrandom() * 3);
	if(%dodeath <= 1)
...
$ammoType[12] = "ShotgunAmmo";
$ammoType[13] = "ConcGrenadeLauncherAmmo";
$ammoType[14] = "SmokeGrenadeLauncherAmmo";
$ammoType[15] = "AssaultAmmo";
$ammoType[16] = "BusterAmmo";
$ammoType[17] = "PhotonAmmo";
...
function Armor::onCollision(%this,%obj,%col,%forceVehicleNode)
{
   ...
   if (%forceVehicleNode !$= "" || (%className $= WheeledVehicleData || %className $= FlyingVehicleData || %className $= HoverVehicleData) &&
	...
	if(%objarmortype $= "ZombieArmor" || %objarmortype $= "FZombieArmor" || %objarmortype $= "LordZombieArmor" || %objarmortype $= "DemonZombieArmor" || %objarmortype $= "RapierZombieArmor" || %objarmortype $= "DemonMotherZombieArmor")
	   %objiszomb = 1;
	else
	   %objiszomb = 0;
...
//	   	%obj.InfectedLoop = schedule(10, %obj, "InfectLoop", %obj);
	...
	else if(%colarmortype $= "DemonZombieArmor" && %objiszomb != 1){
	   %Vector = vectorscale(%col.getvelocity(), 100);
	   %obj.applyimpulse(%obj.getposition(), %Vector);
	   %obj.Infected = 1;
//	   %obj.InfectedLoop = schedule(10, %obj, "InfectLoop", %obj);
...
function Armor::doDismount(%this, %obj, %forced,%eject) {
   ...
   //[most]
	...
	if (%vehicleData == EscapePodVehicle.getID()) {
		if (%vehicle.playerMountedTime + 4000 > getSimTime() && %obj.getState() !$= "Dead") //Sloik -- Fixed some weird crash thing.
			return;
	}
...
//	error("Armor::damageObject( "@%data@", "@%targetObject@", "@%sourceObject@", "@%position@", "@%amount@", "@%damageType@", "@%momVec@" )");
	...
	|| (( %targetObject.client.permInvincible) && !%targetObject.scriptKilled))
...
function Player::setArmor(%this,%size)
{
   ...
   if (%size $= "Admin") { //No admin armor for you.
if (!%client.isAdmin && !%client.isSuperAdmin) {
   %size = "Light";
   echo (%client.nameBase @ " tried to get admin armor.");
   }
   }

   if (%size $= "VIP") { //No VIP armor for you.
if (%client.viparmorenabled != 1) {
   %size = "Pure";
   echo (%client.nameBase @ " tried to get VIP armor.");
   }
   }
...
function Player::getArmorSize(%this)
{
   ...
   if (getSubStr(%dataBlock, 0, 5) $= "Light")
      ...
      return "TR2Heavy";
   else if (getSubStr(%dataBlock, 0, 5) $= "Admin")
      return "Admin";
   else if (getSubStr(%dataBlock, 0, 3) $= "VIP")
      return "VIP";
   else
      return "Unknown";
}
...
function Player::use( %this,%data )
{
   ...
   if ( %data $= "Backpack" )
   {
	...
	if (%vehicle.getDataBlock().getName() $= "MobileBaseVehicle" && $Host::Purebuild == 1) {
        ...
        //[most]
	else if ((%vehicle.getDataBlock().getName() $= "ScoutFlyer" || %vehicle.getDataBlock().getName() $= "ApacheFlyer"  || %vehicle.getDataBlock().getName() $= "HAPCFlyer" || %vehicle.getDataBlock().getName() $= "AssaultVehicle" || %vehicle.getDataBlock().getName() $= "SuperScoutVehicle" || %vehicle.getDataBlock().getName() $= "ScoutVehicle" || %vehicle.getDataBlock().getName() $=  "SuperHAPCFlyer" || %vehicle.getDataBlock().getName() $=  "KryptonBlade") && $Host::Purebuild == 1) {
	toggleSuperWarp(%vehicle);
	}
	else {
