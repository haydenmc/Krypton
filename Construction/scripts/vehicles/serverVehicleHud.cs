...
//	}
	...
	if ( %station.vehicle[ApacheFlyer] ) {
		%vehicleSet[ApacheFlyer] = %i;
		%i++;
	}
	if ( %station.vehicle[KryptonBlade] ) {
		%vehicleSet[KryptonBlade] = %i;
		%i++;
	}
...
//	}
	...
	if ( checkVehSet(%vehicleSet[ApacheFlyer], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "APACHE FLYER", "", ApacheFlyer, $VehicleMax[ApacheFlyer] - $VehicleTotalCount[%team, ApacheFlyer] );
		%count++;
	}
	if ( checkVehSet(%vehicleSet[KryptonBlade], %initPos)) {
		messageClient( %client, 'SetLineHud', "", %tag, %count, "Krypton Blade", "", KryptonBlade, $VehicleMax[KryptonBlade] - $VehicleTotalCount[%team, KryptonBlade] );
		%count++;
	}
