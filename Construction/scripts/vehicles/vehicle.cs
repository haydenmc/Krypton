...
function clearVehicleCount(%team)
{
   ...
   $VehicleTotalCount[%team, ApacheFlyer]         = 0;
   $VehicleTotalCount[%team, KryptonBlade]         = 0;
}
...
function findEmptySeat(%vehicle, %player, %forceNode)
{
   ...
   {
      if (%player.client.armor $= "Light" || %player.client.armor $= "Pure" || %player.client.armor $= "Admin" || %player.client.armor $= "VIP")
         %minNode = 0;
      else
         %message = '\c2Only Scout Armors can pilot this vehicle.~wfx/misc/misc.error.wav';
   }
   else if (%player.client.armor $= "Light" || %player.client.armor $= "Medium" || %player.client.armor $= "Pure" || %player.client.armor $= "Admin" || %player.client.armor $= "VIP")
      %minNode = 0;
   else
