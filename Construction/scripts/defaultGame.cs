...
%client.player.setPosition(%client.spawnposition); //Spawn at specified Krypton Preferences spawn point.

	if ($Host::Prison::Enabled == true) {
		if (%client.isJailed)
			// If player should manage to get out of jail, re-spawn and NO --> re-start sentence time
			//jailPlayer(%client,false,mAbs(%client.jailTime)); //Oh my god, bug fixed. -- SLoik

	if (isObject(%client.player)) {
		if (%client.player.getState() !$="Dead") {
			if(%client.player.isMounted()) {
				if(%client.player.vehicleTurret)
					%client.player.vehicleTurret.getDataBlock().playerDismount(%client.player.vehicleTurret);
				else {
			...
			%client.player.setInventory(EnergyPack,1,1); // Fix Satchel Charge
			%client.player.clearInventory();
			%client.setWeaponsHudClearAll();
			%client.player.setArmor("Light");
			Game.dropFlag(%cl.player);
			%client.preJailVel = %client.player.getVelocity();
			%client.preJailPos = %client.player.getPosition();
			...
			if (($Host::Prison::JailMode $= "0" || $Host::Prison::JailMode $= "")
				...
				if ($Prison::JailRad > 0) {
					...
					%x = %x + (mSin(%vec) * %rad);
					%y = %y + (mCos(%vec) * %rad);
					%client.player.setPosition("-0.19715 2.69382 10011");
				}
				else
					%client.player.setPosition($Prison::JailPos);
			}
			else if ($Host::Prison::JailMode == 1) {
				%client.player.setVelocity("0 0 0");
				%client.player.setPosition(Game.pickPlayerSpawn(%client,false));
			}
			else {
...
//Is their aiming laser enabled?
if (%player.client.laserMode == 1) {
    %plyr = %player;
...
function DefaultGame::onClientDamaged(%game, %clVictim, %clAttacker, %damageType, %sourceObject)
{
   ...
   if (%sourceClassType $= "TurretData")
   {
	  ...
	  // sloik: Quiet! All of you! Turrets should attack MTCs, too!.

      if ((isDeployedTurret(%sourceObject)) || (%sourceType $= "LightmtcTurret" || %sourceType $= "MediumMtcTurret" || %sourceType $= "HeavyMtcTurret" || %sourceType $= "HugeMtcTurret"))
      {
...
//Make sure the aiming laser goes away...
   if(isObject(%plVictim.attachedLaser)) {
      %plVictim.attachedLaser.delete();
//      %plVictim.client.laserMode = 0;
      %plVictim.client.laserActive = 0;
}
   if(isObject(%plVictim.DeathBeam)) {
      %plVictim.DeathBeam.delete();
//      %plVictim.client.laserMode = 0;
      %plVictim.client.laserActive = 0;
}
   
//Theorem cries out in pain when he dies.
if (%clVictim == $tid) {
if (getRandom(0,1)) {
schedule(250,0,cannedChatMessageAll,$tid,"\c4[Kr] Theorem: Ow!.~wavo.deathcry_01", "[Kr] Theorem", "\c4[Kr] Theorem: Ow!.~wavo.deathcry_01", "BOT"); //ouch!
} else {
schedule(250,0,cannedChatMessageAll,$tid,"\c4[Kr] Theorem: Ack!.~wavo.deathcry_02", "[Kr] Theorem", "\c4[Kr] Theorem: Ack!.~wavo.deathcry_02", "BOT"); //ouch!
}
}
...
echo(%clVictim.nameBase @ " was killed by " @ %clKiller.nameBase);
...
%dontjail = false;
//Look for combat zones!
		// Extra safety
		if (%clKiller.player.incombatsphere !$= "" && %clVictim.player.incombatsphere !$= "") { //It's a combat sphere, and we're BOTH in its range.
			%dontjail = true; //Make sure they're not jailed!
			}

	if ($Host::Prison::Enabled == true && %clKiller != %clVictim
	    && ($Host::Prison::Kill == true || (%clKiller.team == %clVictim.team && $Host::Prison::TeamKill == true))
//	    && %clKiller.player // Make sure killer is a player (NO)
	    && isObject(%clKiller)
            && %clVictim != %clKiller
	    && !%dontjail //Part of the combat zone lookup
