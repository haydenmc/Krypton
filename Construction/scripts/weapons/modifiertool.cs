...
function MTScaleShiftMerge(%piece1, %piece2)
{
	...
	disassemble(0, %piece2.getOwner(), %piece2); //disassemble is cleaner
}
...
datablock ShapeBaseImageData(ModToolImage)
{
...
function ModToolImage::onFire(%data,%obj,%slot)
{
...
function ModToolImage::onMount(%this,%obj,%slot)
{
...
if (%obj.modifierMode $= "")
   %obj.modifierMode = 0;
	...
	%obj.mountImage(ModToolImage, 0);
%client = %obj.client;
//CommandToClient(%client, 'BottomPrint', "<font:Sui Generis:14>>>>Modifier Tool<<<\n<font:Arial:14>" @ %status @ "\nCoded by Electricutioner, modified by CaptnPower and Sloik.", 3, 3 );

if (%obj.modifierMode ==2)
%line2 = getwords($packSetting["forcefield",%obj.modifierMode2],5,19);
else if (%obj.modifierMode ==1)
%line2 = getwords($WeaponSetting["modifier1",%obj.modifierMode2],1,19);
else if (%obj.modifierMode ==3)
%line2 = $WeaponSetting["modifier2",%obj.modifierMode2];
else if (%obj.modifierMode ==4)
%line2 = $WeaponSetting["modifier3",%obj.modifierMode2];
else if (%obj.modifierMode ==5)
%line2 = $WeaponSetting["modifier4",%obj.modifierMode2];
else
%line2 = $WeaponSetting["modifier0",%obj.modifierMode2];

%line1 = $WeaponSetting2["modifier",%obj.modifierMode];
%wepname = "Modifier Tool";
//%mode1 = $WeaponSetting["modifier",%obj.modifierMode];
//%mode2 = $WeaponSetting["modifier" @ %obj.modifierMode,%obj.modifierMode2];
displaywepstat(%obj.client,%wepname,%line1,%line2);
}
function ModToolImage::onUnmount(%data, %obj, %slot) {
...
%sender=%plyr.client;
if ( %hitobj.getOwner().guid != %sender.guid){
   if (!%sender.isadmin && !%sender.issuperadmin){
      if (%hitobj.getOwner().guid !$=""){
