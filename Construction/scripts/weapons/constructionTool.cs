...
$ReverseDeployItem[DeployedTDoor] = "poof TDoorDeployable";
...
$ReverseDeployItem[DeployedDecoy] = "DecoyDeployable";
$ReverseDeployItem[TurretDeployedrepair] = "repairTurretDeployable";
...
$ReverseDeployItem[DeployedOOSphere] = "PermSphereDeployable";
$ReverseDeployItem[DeployedPeaceSphere] = "PermSphereDeployable";
$ReverseDeployItem[DeployedCombatSphere] = "PermSphereDeployable";
$ReverseDeployItem[DeployedProximitySwitch] = "ProximitySwitch";
$ReverseDeployItem[DeployedEditorPack] = "EditorPackDeployable";
...
datablock RepairProjectileData(ConstructionToolBeam) {
	...
	textures[0]   = "special/sniper00";
	textures[1]   = "skins/hud_ret_bomber1";
};

//-------------------------------------------------------------------------
// shapebase datablocks

datablock TargetProjectileData(ConstructionTargeter)
{
   directDamage        	= 0.0;
...
datablock ShapeBaseImageData(ConstructionScopeImage)
{
   ...
   projectile = ConstructionTargeter;
...
function ConstructionToolImage::onMount(%this,%obj,%slot) {
	...
	%mode1 = $WeaponSetting["ConstructionTool",%obj.constructionToolMode];
	%mode2 = $WeaponSetting["ConstructionTool" @ %obj.constructionToolMode,%obj.constructionToolMode2];
	displaywepstat(%obj.client,%wepname,%mode1,%mode2);
	%obj.mountImage(ConstructionScopeImage, 4); //The scope
		...
		if (%canPerform == true) {
			...
			if (%hTgt != %obj.performing) { // its not what we were originally rotating
				...
				if (%obj.performTime < 0) { // we have a rot!
					...
					if (%player.rotationamount !$= "")
					  %rotVal = ((%player.rotationamount/360)*2*8)*%rotVal;
