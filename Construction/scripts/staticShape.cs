...
function shouldChangePowerState(%obj,%state) {
	if (%obj.lastState $= "")
		return true;
	if (%obj.isSwitchedOff == true)
		return false;
	return %obj.lastState != %state;
}

function StaticShapeData::onCollision(%data,%obj,%col) {
if (%obj.isdoor !=true && %obj.getdatablock().getname() !$="DeployedTdoor")
   return;
...
%loc1 = findWord(%client.lv1,%owner);
%loc2 = findWord(%client.lv2,%owner);
%loc3 = findWord(%client.lv3,%owner);
////////////////////
//door level chek //
////////////////////
if (%obj.lv == 1 && %ownerguid != %client.guid){
   if (%loc1 $= "" && %loc2 $= "" && %loc3 $= ""){
      messageclient(%client, 'MsgClient',"\c2Access denied! Get a level \c31\c2 access card from \c3"@%owner.namebase );
      return;
      }
   }
else if (%obj.lv == 2 && %ownerguid != %client.guid){
     if (%loc2 $= "" && %loc3 $= ""){
        messageclient(%client, 'MsgClient',"\c2Access denied! Get a level \c32\c2 access card from \c3"@%owner.namebase );
        return;
        }
     }
else if (%obj.lv == 3 && %ownerguid != %client.guid){
     if (%loc3 $= ""){
        messageclient(%client, 'MsgClient',"\c2Access denied! Get a level \c33\c2 access card from \c3"@%owner.namebase );
        return;
        }
     }
else if (%obj.lv == 4 && %ownerguid != %client.guid){
        messageclient(%client, 'MsgClient',"\c2Access denied! Access is restricted to \c3"@%owner.namebase );
        return;
     }
else if (%obj.lv == 5 && %ownerguid != %client.guid && !%client.isadmin){
        messageclient(%client, 'MsgClient',"\c2Access denied! Access is restricted to \c3administrators.");
        return;
     }
else if (%obj.lv == 6 && %ownerguid != %client.guid && !%client.issuperadmin){
        messageclient(%client, 'MsgClient',"\c2Access denied! Access is restricted to \c3super administrators.");
        return;
     }
////////////////
//name of door//
...
if (%obj.lv == 1)
   ...
   %lv = "Collision";
//////////////////////////////////////////////////////////////////
//if u have the right key or the door is simply on contact open //
//////////////////////////////////////////////////////////////////
    //messageclient(%client, 'MsgClient',"\c2acces \c5granted \c2 to \c5"@%lv@" door");
    if (%obj.timeout $="")
       %obj.timeout=1;
...
if (%data.needsPower) {
%obj.setRechargeRate(%data.rechargeRate + ((%obj.powerCount - 1) * (%data.rechargeRate * 0.5) ));
}
else {
...
if (%obj.isdoor == 1 || %obj.getdatablock().getname() $="DeployedTdoor") {
if (%obj.canmove == false) //if it cant move
   return;                  //stop here
if (%obj.powercontrol == 0)
   return;
if (%obj.toggletype ==1) {
   if (%obj.powercontrol == 2){
         if (%obj.state $="opened"){
             schedule(10,0,"close",%obj);
             }//
         return;
         }//if (%obj.toggletype ==1 && %obj.powercontrol == 2){
   else if (%obj.powercontrol == 3){
         if (%obj.state $="closed"){
             schedule(10,0,"open",%obj);
             }//
         return;
         }//else if (%obj.toggletype ==1 && %obj.powercontrol == 3){
   else if(%obj.powercontrol == 1){
...
if (%obj.powercontrol == 1  && %obj.toggletype == 0)
    schedule(10,0,"open",%obj);
}//if (%obj.isdoor == 1 || %obj.getdatablock().getname() $="DeployedTdoor") {
}
...
if (%obj.powercontrol == 0)
   return;
if (%obj.toggletype ==1) {
   if (%obj.powercontrol == 3){
        if (%obj.state $="opened"){
             schedule(10,0,"close",%obj);
             }//
         return;
         }//if (%obj.toggletype ==1 && %obj.powercontrol == 3){
   else if (%obj.powercontrol == 2){
         if (%obj.state $="closed"){
             schedule(10,0,"open",%obj);
             }//
         return;
         }//else if (%obj.toggletype ==1 && %obj.powercontrol == 2){
   else if (%obj.powercontrol == 1){
      if (%obj.moving $="close" || %obj.moving $="" || %going $="opening"){
         schedule(10,0,"open",%obj);
         }
...
function StaticShape::onLoad(%this)
{

if (%this.dataBlock $= "GeneratorLarge")
{
echo("Loading Generator...");

//setTargetSensorGroup(%this.getTarget(),mAbs(%this.team));
addToDeployGroup(%this);
%this.setSelfPowered();
setTargetName(%this.target,addTaggedString("Frequency" SPC %this.powerFreq));
if (%this.lastState $= "1") {
%this.playThread($PowerThread,"power");
} else {
%this.playThread($AmbientThread,"ambient");
}
%this.depTime = "";
%this.switchTime = "";
}

if (%this.dataBlock $= "SolarPanel")
{
echo("Loading Solar panel...");
...
if (%this.dataBlock $= "DeployedSwitch")
{
%this.setSelfPowered();
setTargetName(%this.target,addTaggedString("Disabled Frequency" SPC %this.powerFreq));
setTargetSkin(%this.target,'');
%this.lastState = "";
...
if (%this.dataBlock $= "TelePadDeployedBase")
{
echo("Loading Telepad...");
//setTargetSensorGroup(%this.getTarget(),mAbs(%this.team));
addToDeployGroup(%this);
...
%this.beam = %beam;
%beam.playThread(0,"ambient");
%beam.setThreadDir(0,true);
%beam.flashThreadDir = true;
tp_adjustBeam(%this);
setTargetName(%this.target,addTaggedString("Frequency" SPC %this.frequency));
}

if (%this.dataBlock $= "DeployedEscapePod")
{
echo("Loading escape pod...");
%this.lTarget = new StaticShape () {
	datablock = "DeployedLTarget";
	scale = vectorMultiply("1 3 0.25",1/4 SPC 1/3 SPC 2);
	lMain = %this;
};
addToDeployGroup(%this.lTarget);
%this.setSelfPowered();

	%lTarget = %this.lTarget;
	%pos = %this.getPosition();
	%rot = %this.getRotation();
	%nrm = realVec(%this,"0 0 -1");
	%this.lTarget.setTransform(vectorAdd(%pos,vectorScale(%nrm,1.25)) SPC %rot);

escapePodLoop(%this);
}

if (%this.dataBlock $= "DeployedTripwire")
{
	%this.tripField = new ForceFieldBare() {
		dataBlock = TripField;
		scale = "0.05 0.05 0.1";
	};

	%this.tripField.pzone.delete();

	%this.tripTrigger = new Trigger() {
		dataBlock = TripTrigger;
		polyhedron = "0 1 0 1 0 0 0 -1 0 0 0 1";
		scale = "0.05 0.05 0.1";
	};

	%this.tripTrigger.baseObj = %this;

	adjustTripwire(%this);

	// set the recharge rate right away
	if (%this.getDatablock().rechargeRate)
	%this.setRechargeRate(%this.getDatablock().rechargeRate);

setTargetName(%this.target,addTaggedString("Frequency" SPC %this.powerFreq));
}

if (%this.dataBlock $= "DeployedLightBase")
{
	%this.light = new Item() {
		datablock = DeployedLight @ %this.lighttype;
		static = true;
	};
	...
	if (%this.getDatablock().rechargeRate)
		%this.setRechargeRate(%this.getDatablock().rechargeRate);
	%this.light.lightBase = %this;
}

if (%this.dataBlock $= "DeployedProximitySwitch")
{
	setTargetName(%this.target,addTaggedString("Frequency" SPC %this.powerFreq));
	%this.deploy();
	%this.playThread($AmbientThread, "ambient");
schedule(128,0,ProximityCheck,%this);
}

if (%this.dataBlock $= "Deployedwaypoint")
{
     %wp = new  (WayPoint)(){
...
if (%this.dataBlock $= "TurretDeployedOutdoor" || %this.dataBlock $= "TurretDeployedFloorIndoor" || %this.dataBlock $= "TurretDeployedWallIndoor" || %this.dataBlock $= "TurretDeployedCeilingIndoor"  || %this.dataBlock $= "TurretDeployedrepair")
{
%this.deploy();
%this.setSelfPowered();
%this.setRechargeRate(%this.getDatablock().rechargeRate);
...
if (%this.dataBlock $= "DeployableVehicleStation") {
checkPowerObject(%this);
//adjustTrigger(%this);
%this.deploy(); 
}
...
if (%this.dataBlock $= "DeployedProximitySwitch" || %this.dataBlock $= "DeployedTripwire" ||  %this.dataBlock $= "DeployedEscapePod" || %this.dataBlock $= "TelePadDeployedBase" || %this.dataBlock $= "DeployedSwitch" || %this.dataBlock $= "SolarPanel" || %this.dataBlock $= "GeneratorLarge" ||  %this.dataBlock $= "DeployedMotionSensor" || %this.dataBlock $= "TurretDeployedBase" || %this.dataBlock $= "TurretDeployedFloorIndoor" || %this.dataBlock $= "TurretDeployedWallIndoor" || %this.dataBlock $= "TurretDeployedCeilingIndoor" || %this.dataBlock $= "TurretDeployedOutdoor" || %this.dataBlock $= "Mpm_Anti_TurretDeployed" || %this.dataBlock $= "LaserDeployed" || %this.dataBlock $= "MissileRackTurretDeployed" || %this.dataBlock $= "DiscTurretDeployed" || %this.dataBlock $= "TurretDeployedCamera" || %this.dataBlock $= "DeployedEditorPack" || %this.dataBlock $= "StationInventory" || %this.dataBlock $= "SensorMediumPulse" || %this.dataBlock $= "SensorLargePulse" || %this.dataBlock $= "DeployedPulseSensor" || %this.dataBlock $= "DeployedMotionSensor" || %this.dataBlock $= "DeployableVehicleStation" || %this.dataBlock $= "DeployableVehiclePad" || %this.dataBlock $= "DeployableVehiclePad2" || %this.dataBlock $= "TurretDeployedrepair") { //These can have labels/targets.
setTargetSensorGroup(%this.getTarget(),mAbs(%this.team));
if (%this.label !$= "")
setTargetName(%this.target,addTaggedString("\c8"@%this.label@"\c6"));
}

if (%this.dataBlock $= "DeployedPulseSensor" || %this.dataBlock $= "DeployedMotionSensor") {
%this.deploy();
}
...
{
   if(%thread == $DeployThread && !%data.needsPower)
      %obj.setSelfPowered();
...
function randomOrg(%organicName, %num, %radius)
{
   ...
   while((%orgCount > 0) && (%retries < (15000 / %maxSlope)))  //theoretically, a thorough number of retries
   {
      ...
      %finalXForm = validateVal(MatrixMultiply(%orient, %zrot));
...
function randomRock(%rock, %quantity, %radius, %maxElev)
{
	...
	while((%orgCount > 0) && (%retries < (15000 / %maxSlope)))  //theoretically, a thorough number of retries
	{
		...
		%group.add(%newRock);
		%transfrm = validateVal(MatrixMultiply(%orient, %zrot));
...
function StaticShapeData::damageObject(%data, %targetObject, %sourceObject, %position, %amount, %damageType) {
	...
	if ($Host::InvincibleDeployables == 1) {
 		if (%targetObject.getGroup() == Deployables.getID()
			&& !%data.powerRadius
			&& %data != StationInventory.getID()
			&& !%isFF
			&& !%isTurret)
			return;
	}
	if (isObject(%sourceObject))
		if (%sourceObject.getType() & $TypeMasks::VehicleObjectType)
			if (%sourceObject.getDataBlock() == EscapePodVehicle.getID()) {
				// Reverse damage ;)
				%tmp = %sourceObject;
	...
	if (%targetObject.lTarget) {
		if (isObject(%targetObject.lTarget)) {
			return;
		}
	}
	else if (%targetObject.lMain) {
		if (isObject(%targetObject.lMain)) {
			%targetObject = %targetObject.lMain;
		}
	}

   // if this is a non-team mission type and the object is "protected", don't damage it
   if(%data.noIndividualDamage && Game.allowsProtectedStatics() && !%targetObject.deployed)
      return;
   ...
   if(%data.noDamageInSiege && Game.class $= "SiegeGame")
	...
	// Don't let deployed turrets damage other deployed items of the same team
	if (isObject(%sourceObject) && isObject(%targetObject)) {
		if ((%targetObject.getDataBlock().deployedObject || %targetObject.deployed)
		&& (%sourceObject.getType() & $TypeMasks::TurretObjectType)
		&& %sourceObject.team == %targetObject.team)
			return;
	}

	// Switch to not let deployed turrets damage any deployed item of any team
	if (isObject(%sourceObject) && isObject(%targetObject) && $NoTurretDamage == 1) {
		if ((%targetObject.getDataBlock().deployedObject || %targetObject.deployed)
		&& (%sourceObject.getType() & $TypeMasks::TurretObjectType))
			return;
	}
   ...
   if(%damageScale !$= "")
	...
	if (!$TeamDamage && !%targetObject.getDataBlock().deployedObject) {
		...
		if (((%targetObject.getTarget() != -1) && isTargetFriendly(%targetObject.getTarget(), %attackerTeam)) && !%isFF) {
	...
	if (%amount > 0) {
		if (%isFF)
			%targetObject.applyDamage(%amount,%sourceObject,%position,%damageType);
		else
...
function StaticShapeData::onDestroyed(%data,%obj,%prevState) {
	if(%data.expDmgRadius && !%obj.hasExploded) {
		RadiusExplosion(%obj, %obj.getWorldBoxCenter(), %data.expDmgRadius, %data.expDamage, %data.expImpulse, %obj, $DamageType::Explosion);
		%obj.hasExploded = true;
	}
