...
$TeamDeployableMax[LargeInventoryDeployable] = 9999;
$TeamDeployableMax[GeneratorDeployable] = 9999;
$TeamDeployableMax[SolarPanelDeployable] = 9999;
$TeamDeployableMax[SwitchDeployable] = 9999;
$TeamDeployableMax[MediumSensorDeployable] = 9999;
$TeamDeployableMax[LargeSensorDeployable] = 9999;
$TeamDeployableMax[WallDeployable] = 1000;
$TeamDeployableMax[FloorDeployable] = 1000;
$TeamDeployableMax[wWallDeployable] = 1000;
$TeamDeployableMax[SpineDeployable] = 1000;
$TeamDeployableMax[mSpineDeployable] = 1000;
$TeamDeployableMax[JumpadDeployable] = 50;
$TeamDeployableMax[EscapePodDeployable] = 20;
$TeamDeployableMax[EnergizerDeployable] = 50;
$TeamDeployableMax[TreeDeployable] = 70;
$TeamDeployableMax[CrateDeployable] = 250;
$TeamDeployableMax[DecorationDeployable] = 100;
$TeamDeployableMax[LogoProjectorDeployable] = 50;
$TeamDeployableMax[LightDeployable] = 50;
$TeamDeployableMax[TripwireDeployable] = 50;
$TeamDeployableMax[ForceFieldDeployable] = 999;
$TeamDeployableMax[GravityFieldDeployable] = 99;
$TeamDeployableMax[TelePadPack] = 50;
$TeamDeployableMax[DoorDeployable] = 1000;

$TeamDeployableMax[TurretBasePack] = 100;
$TeamDeployableMax[TurretLaserDeployable] = 100;
$TeamDeployableMax[DiscTurretDeployable] = 100;
$TeamDeployableMax[TurretMissileRackDeployable] = 100;

//This could also be in "pack".cs but what the heck
//[most]
$TeamDeployableMax[TurretMpm_Anti_Deployable] = 40;
$TeamDeployableMax[VehiclePadPack] = 40;
$TeamDeployableMax[EmitterDepPack] = 50;
$TeamDeployableMax[AudioDepPack] = 50;
$TeamDeployableMax[DispenserDepPack] = 50;
$TeamDeployableMax[MPM_BeaconPack] = 50;
$TeamDeployableMax[DetonationDepPack] = 5;
//noone cares
...
$TeamDeployableMin[TurretIndoorDeployable] = 100;
$TeamDeployableMin[TurretOutdoorDeployable] = 100;
$TeamDeployableMin[TurretBasePack] = 100;
$TeamDeployableMin[TurretLaserDeployable] = 100;
$TeamDeployableMin[DiscTurretDeployable] = 100;
$TeamDeployableMin[TurretMissileRackDeployable] = 100;
...
//Other SHTUFF
$TeamDeployableMax[DecoyDeployable] = 9999;
$TeamDeployableMax[PermSphereDeployable] = 9999;
$TeamDeployableMax[ProximitySwitch] = 9999;
...
$NotDeployableReason::SphereSpawn		=  12;
$NotDeployableReason::SphereVehicles		=  13;
$NotDeployableReason::SphereCount		=  14;
$NotDeployableReason::SphereOwnership		=  15;
$NotDeployableReason::ProximityRange		=  16;
$NotDeployableReason::ProximityFreq		=  17;
$NotDeployableReason::VisLimit			=  18;
...
if ($PieceCount > 800) //Over 800 pieces?
%disqualified = $NotDeployableReason::VisLimit;
	...
	if (%pack.getName() $= "PermSphereDeployableImage") {
//SPHERE SHTUFF

if (%plyr.client.spherecount >= 2) {
	%disqualified = $NotDeployableReason::SphereCount;
}

//Range for analysis
	if (%plyr.expertSet == 0)
	%therange = 50; //50 meter range
	if (%plyr.expertSet == 1)
	%therange = 100; //100 meter range
	if (%plyr.expertSet == 2)
	%therange = 300; //300 meter range

//No deploying these near player spawns.
   %teamDropsGroup = "MissionCleanup/TeamDrops" @ %plyr.client.team;
   %group = nameToID(%teamDropsGroup);
   %count = %group.getCount();
 for (%i = 0; %i < %count; %i++)
	 {
	    %sphereObj = %group.getObject(%i);
	if (VectorDist(%sphereObj.getPosition(),%plyr.getPosition()) < (%therange + 75)) {
%disqualified = $NotDeployableReason::SphereSpawn;
}
	 }

if (%plyr.packSet == 1) {
InitContainerRadiusSearch(%plyr.getPosition(),(%therange + 75),$TypeMasks::VehicleObjectType);	 //Look for MPBs
if ((%obj = ContainerSearchNext()) != 0)
%disqualified = $NotDeployableReason::SphereVehicles;
}

if (%plyr.packSet == 0 || %plyr.packSet == 2) { //Owner-Only Construct Zone Sphere! Make sure we're not deploying these on other players' buildings.
InitContainerRadiusSearch(%plyr.getPosition(),%therange+50,$TypeMasks::StaticShapeObjectType | $TypeMasks::ItemObjectType | $TypeMasks::ForceFieldObjectType);
	while((%obj = ContainerSearchNext()) != 0) {
		// Extra safety
		if (%obj.getOwner().guid !$= %plyr.client.guid) {
			%dataBlockName = %obj.getDataBlock().getName();
			if (saveBuildingCheck(%obj) && %dataBlockName !$= "TelePadBeam") { // If it's handled by saveBuilding(), it must be a deployable
				%disqualified = $NotDeployableReason::SphereOwnership;
			}
		}
	}
}
...
//			%item.surfaceNrm2 = realVec(%item.surface,"0 0 1");
	...
	if (%pack.getName() $= "ProximitySwitchDeployableImage") {
//Set the range on this baby
	if (%plyr.packSet == 0)
	%proxrange = 2; //2 meter range
	if (%plyr.packSet == 1)
	%proxrange = 5; //5 meter range
	if (%plyr.packSet == 2)
	%proxrange = 10; //10 meter range
	if (%plyr.packSet == 3)
	%proxrange = 20; //20 meter range
	if (%plyr.packSet == 4)
	%proxrange = 40; //40 meter range
	if (%plyr.packSet == 5)
	%proxrange = 80; //80 meter range
	if (%plyr.packSet == 6)
	%proxrange = 160; //160 meter range

//Proximity Switch shtuff...
InitContainerRadiusSearch(%item.surfacePt,165,$TypeMasks::StaticShapeObjectType);
	while((%obj = ContainerSearchNext()) != 0) {
		// Extra safety
			%dataBlockName = %obj.getDataBlock().getName();
			if (%dataBlockName $= "DeployedProximitySwitch" && %obj.powerFreq $= %plyr.powerFreq && (VectorDist(%obj.getPosition(),%item.surfacePt) < (%proxrange + %obj.range + 5))) {
				%disqualified = $NotDeployableReason::ProximityRange;
			}
	}
}
...
function Deployables::displayErrorMsg(%item, %plyr, %slot, %error)
{
   ...
   switch (%error)
   {
      ...
      case $NotDeployableReason::SphereSpawn:
         %msg = '\c2Interference from a nearby spawn point prevents placement here. Your sphere must not ecompass a spawn point.%1';

      case $NotDeployableReason::SphereVehicles:
         %msg = '\c2Interference from a nearby vehicle prevents placement here. Your sphere must not ecompass a vehicle.%1';

      case $NotDeployableReason::SphereCount:
         %msg = '\c2You have reached the limit of permission spheres.%1';

      case $NotDeployableReason::SphereOwnership:
         %msg = '\c2Interference from nearby deployables prevents placement here. Your sphere must not ecompass other players\' deployables.%1';

      case $NotDeployableReason::ProximityRange:
         %msg = '\c2Interference from a nearby proximity switch prevents placement here.%1';

      case $NotDeployableReason::ProximityFreq:
         %msg = '\c2Only one proximity switch is allowed per power frequency.%1';

      case $NotDeployableReason::VisLimit:
         %msg = '\c2The server has reached the visual limit.%1';
...
// Let's try this.... only allow ONE!
function disassemble(%data,%plyr,%obj) {
	if (!isObject(%obj))
		return;

	if (%obj.dassembling == 1) //Already disassembling?
		return;		//Don't do it again.

//Save this piece to the player's undo file...
addUndoPiece(%plyr.client,%obj);
saveundofile(%plyr.client);


%obj.dassembling = 1; //We are now disassembling.
schedule(5000,0,dassemblereset,%obj); //In 5 seconds, make it dissassemble-able...
...
function dassemblereset(%obj) //Just a function to reset a variable.
{
%obj.dassembling = 0; //Allow for disassembly now...
}

function deleteClientPieces(%client) {
schedule(10,0,"deleteClientPieces2",%client);
schedule(1000,0,"deleteClientPieces2",%client);
schedule(2000,0,"deleteClientPieces2",%client);
}

function deleteClientPieces2(%client) {
%group = nameToID("MissionCleanup/Deployables");
	%count = %group.getCount();
  for(%i=0;%i<%count;%i++) {
		%obj =  %group.getObject(%i);
	 ...
	 %random = getRandom(1000,2000);
     %datablockName = %obj.getDatablock().getName();
      if(strStr(%datablockName,"DeployedForceField") != -1) {
      DeployedForceField::disassemble(%obj.getDatablock(),%client.player,%obj);
      %dontdecon = 1;
      }
      if(strStr(%datablockName,"DeployedGravityField") != -1) {
      DeployedGravityField::disassemble(%obj.getDatablock(),%client.player,%obj);
      %dontdecon = 1;
      }
     if(!%dontdecon)
//		 %obj.getDataBlock().schedule(%random,"disassemble",%client.player,%obj);
		 %obj.getDataBlock().disassemble(%client.player,%obj);
     }
    }

}

function deleteGUIDPieces(%theguid,%client) {
%group = nameToID("MissionCleanup/Deployables");
	%count = %group.getCount();
  for(%i=0;%i<%count;%i++) {
		%obj =  %group.getObject(%i);
     if(%obj.getOwner().guid == %theguid) {
     warn("Deleting: " @ %obj @ " Name: " @ %obj.getDataBlock().getName());
	 %random = getRandom(1000,2000);
     %datablockName = %obj.getDatablock().getName();
     if(!%dontdecon)
...
schedule(10,0,"delOrphanedPieces2",%quiet);
schedule(1000,0,"delOrphanedPieces2",%quiet);
schedule(2000,0,"delOrphanedPieces2",%quiet);
}

function delOrphanedPieces2(%quiet) {
	%randomTime = 10000;
	%group = nameToID("MissionCleanup/Deployables");
	%count = %group.getCount();
	for(%i=0;%i<%count;%i++) {
		%obj =  %group.getObject(%i);
		if (!isObject(%obj.getOwner()) && %obj.getOwner() !$= "1" && %obj.getDataBlock().getName() !$= "DeployedSpawnSphere") {
			if (!%quiet)
				warn("Deleting: " @ %obj @ " Name: " @ %obj.getDataBlock().getName());
			%random = getRandom() * %randomTime;
			%obj.getDataBlock().schedule(%random,"disassemble",%plyr,%obj); // Run Item Specific code.
